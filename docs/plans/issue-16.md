...
Issue: [https://github.com/f13r/gymtracker/issues/16](https://github.com/f13r/gymtracker/issues/16) ("Create superset functionality")
...
- **Adopt "Superset" as the single canonical term for 2+ Exercises.** The user
  ...
  / giant set into "a larger Superset" — no separate term. `_Avoid_: circuit, tri-set, giant set, drop set` (note "drop set" is a genuinely different concept,
...
_membership_ by `orderIndex` and works identically whether or not other exercises
sit between members. So contiguity is **not** a data-model requirement — it is
purely an editor-interaction / rendering preference:

- _Require contiguous_ → editor = "select a run of rows → group"; `supersetGroup`
  derivable from position; one solid color block per group; matches the physical
  "back-to-back" intuition.
- _Allow non-adjacent_ → more flexible; the model supports it natively; members
...
color rendering), but record in the CONTEXT.md Superset term that the _data model_
...
Human answer: _Require contiguous_
...
- `apps/api` and `packages/shared` vitest suites pass; `pnpm --filter @gymtracker/web lint` + web build pass; `react-doctor` regression is clean for
  changed web files.
...
- [x] **Red:** extend `apps/api/src/workouts/workouts.service.spec.ts` (or the
...
- [x] Add nullable `supersetGroup` to the Drizzle schema on **both**
...
- [x] Generate the additive migration (`drizzle-kit generate`) into
...
- [x] Add `supersetGroup: <id> | null` to `TemplateExercise`
      (`packages/shared/src/models.ts:26-35`) and `SessionExercise`
      (`models.ts:66-72`).
- [x] Add `supersetGroup` (optional/nullable) to the create-Template exercise
      ...
      (column stays null).
- [x] Copy `supersetGroup` in `snapshotPlan()`
      ...
      `templateExercises`.
- [x] **Green:** run `apps/api` + `packages/shared` vitest; confirm the new
      assertions pass and no existing test regresses.
- [x] Confirm **no stats code reads `supersetGroup`** (grep stats utils /
      services) — it must stay structure-only (Decisions: stats-neutral).
- [x] Do NOT touch the editor UI or the logger here — those are Tasks 2–4.
...
- [x] **Confirm GATE #1 is resolved** before starting. Resolved: _Require
      contiguous_ (plan line 140). Grouping is expressed contiguity-by-construction —
...
- [x] Add a **Superset** entry to CONTEXT.md (Workout domain section): "An ordered
      ...
      canonical term).
- [x] Record the **GATE #1 contiguity** outcome in that term: state explicitly that
...
- [x] Add a CONTEXT.md Relationships bullet: a Template's Exercises may be grouped
      into Supersets via `supersetGroup`; the grouping is snapshotted into the
      Session at Start and is stats-neutral.
- [x] **Iff GATE #1 adopts contiguity as a real rule**, add an ADR under
      ...
      no ADR.
- [x] Note the deferred seams already identified (no work now): in-session
