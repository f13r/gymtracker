...
**Superset**:
An ordered subset of a Template's Exercises performed back-to-back, one Set each per round. Represented as a nullable `supersetGroup` marker on a Template's exercises (shared id = same Superset; null = standalone), not a standalone entity — order within the group comes from the existing exercise order (`orderIndex`). A Template may contain multiple Supersets. Like all structure, the grouping is copied into the Session Snapshot at Start and read from the Session thereafter; it is **stats-neutral** (orthogonal to every performance metric). In the logger, marking a Set Done inside a Superset advances the **Active Exercise** round-robin to the next member that still owes a **Planned Set**, wrapping to the first; when the whole group has no remaining Planned Sets it returns to the **Overview**. **Contiguity is an editor rule, not a data-model requirement**: the data model (`supersetGroup` id + `orderIndex`) supports non-adjacent members natively, but v1's editor enforces contiguous members for a simpler interaction and cleaner color rendering — so relaxing to non-adjacent Supersets later is pure UI, no migration.
_Avoid_: circuit, tri-set, giant set, drop set

**Permanent Add**:
When the user adds an Exercise to an Active Session, an opt-in choice — surfaced as a checkbox "Add to `<Template name>` permanently" — to _also_ append that Exercise to the **source Template**, so every future Session from that Template includes it. Default off: an unchecked add affects **this Session only** (the normal rule that in-session edits never reshape the Template). Available only when the Session has a `templateId` (Template-started or Session Repeat); hidden for a freeform Session, which has no Template to write to. The Template write commits when the user saves the add, not on selection.
...
- A **Template**'s **Exercises** may be grouped into one or more **Supersets** via a nullable `supersetGroup` marker (shared id = same Superset; null = standalone); the grouping is snapshotted into the **Session** at Start and is **stats-neutral**
