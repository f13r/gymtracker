...
This means a Template edit propagates to all *future* scheduled days that reference it (they read the edited Template when started), while *past* logged workouts are unaffected because their Sets are already snapshotted. A session already in progress reads the Template live, so its not-yet-logged default values can shift, but anything already logged stays — accepted as a negligible edge case for a single user.
