...
- **Deterministic engine rejected:** APRE/Double-Progression are per-exercise rules that can't do the cross-exercise, whole-program holistic reasoning the app is built around (e.g. respecting total muscle-group load across exercises). The LLM's judgement is the product. Determinism is added as *guardrails/signals*, not as the prescriber.
- **ExerciseDB rejected:** its `bodyPart`/`target`/`secondaryMuscles` taxonomy conflicts with our deliberately coarse Category model, and it adds an external runtime dependency. For a personal tracker the right swap target is an exercise from the user's *own* library, not a random external entry.
